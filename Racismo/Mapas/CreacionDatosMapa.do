...
gen obs = _n
spmap odio_nacional using INEGI_Entidad__shp, id(obs) fcolor(Reds2) ndfcolor(dimgray) ndo(black) legstyle (1) name(Grafica_1, replace) title  (% Mensajes de odio septiembre participación nacional)
spmap odio using INEGI_Entidad__shp, id(obs) fcolor(Reds2) ndfcolor(dimgray) ndo(black) legstyle (1) name(Grafica_1, replace) title  (% Mensajes de odio septiembre participación regional)
